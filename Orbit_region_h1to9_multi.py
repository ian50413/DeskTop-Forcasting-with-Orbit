...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 21 14:19:18 2022
...
# roadmap_df_ori=pd.read_csv('MONTHLY_DT_ROADMAP_FOB (1).csv', index_col=0)
roadmap_df_ori = roadmap_df_ori[['time_period','region','roadmap_group','roadmap_invoice_qty',
                                 'roadmap_invoice_qty_lag_1']]
                                  # ,'roadmap_invoice_qty_lag_12']]
...
for hrz in range(1,10):
    ...
    # for _, roadmap_df in df.groupby(by=['region','roadmap_group'])
        ...
        dlt = DLT(
            ...
            regressor_col=['roadmap_invoice_qty_lag_1'],
            seasonality=12,
            estimator='stan-map',
        )
...
"""
beta['Region'] = 'CHINA'
beta.to_csv('Orbit_full_h_CHINA_lag1.csv')
#%%Create new column for each horizon
# alpha={}
# beta=pd.DataFrame()
alpha[f'alpha_{hrz}']=fcst_output.copy()
